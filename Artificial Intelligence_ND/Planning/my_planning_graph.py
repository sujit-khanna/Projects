...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            a_node = PgNode_a(action)  ##Builds an Action node for the corresponding action
            if a_node.prenodes.issubset(self.s_levels[level]):  ##Checks if the precondition is satisfied by previous state
                for state_node in self.s_levels[level]:
                    state_node.children.add(a_node) ##makes the action node child of previous state node   
                    a_node.parents.add(state_node) ##makes the previous state node parent of the action node
                act_nodes.append(a_node)
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            for s_node in a_node.effnodes:    ## adding effect nodes of previous action to state nodes
                state_nodes.add(s_node)
                s_node.parents.add(a_node) ##Make previous action node a parent of this effect node
                a_node.children.add(s_node) ##Make this effect node a child of the previous action node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        ### Finds if node_a1 Effects are cancelled by node_a2 Effects 
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

            
        ### Finds if node_a2 Effects are cancelled by node_a1 Effects    
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        ##Finds if node_a1 Effects Interfers with node_a2 preconditions  
        for effect in node_a1.action.effect_add:
        ...
        ##Finds if node_a2 Effects Interfers with node_a1 preconditions
        for effect in node_a2.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        ## Finds if node_a1 and node_a2 preconditions are mutually exclusive
        for precondition_act1 in node_a1.parents:
            for precondition_act2 in node_a2.parents:
                if precondition_act1.is_mutex(precondition_act2):
                    return True        
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        ## Finds if  two state literals are negation of each other
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        ##Testing for mutual exclusiveness of node_s1 and node_s2
        for s1_precondition in node_s1.parents:
            for s2_precondition in node_s2.parents:
                if not s1_precondition.is_mutex(s2_precondition):
                    return False
